...
class Wizard_create_print_etiq(models.TransientModel):
    ...
    def _onchange_product_ccp(self):
        ...
        if self.category_id:
            product_domain = [
                ('categ_id', '=', self.category_id.id)] + product_domain[0:]
        if self.caliber_id:
            product_domain = [
                ('di_caliber_id', '=', self.caliber_id.id)] + product_domain[0:]
        if self.packaging_id:
            product_domain = [
                ('di_packaging_id', '=', self.packaging_id.id)] + product_domain[0:]
    ...
    def _onchange_partner_product(self):
        ...
        if self.partner_id:
            ...
            id_partner_sender = 0
            codbar_128 = ""
            partner_sender = self.env['res.partner'].search(
                [('id', '=', self.partner_id.di_sender_establishment.id), ])
               ...
               # if not self.health_number:
                ...
                if self.partner_id.di_ean128:
                    if sender.di_code_ean128:
                        codbar_128 = ("%s%08d") % (
                            sender.di_code_ean128, compt_ean128)
        ...
        if (self.product_id) and (self.partner_id):
            ...
            default_value = self.env['di.printing.parameter'].search(
                [('name', '=', 'DEFAULT')])
    ...
    def _compute_dluo(self):
        if self.packaging_date and self.product_id.id:
            ...
            val_calcul_dluo = fields.Date.from_string(
                self.packaging_date) + timedelta(days=val_nb_day)
    ...
    sending_date = fields.Date(
        string="Sending date", default=lambda self: fields.Date.today())
    ...
    category_id = fields.Many2one('product.category', 'Internal Category', domain=[
                                  ('parent_id', '!=', False), ('di_shell', '=', True)], store=False)
    caliber_id = fields.Many2one('di.multi.table', string='Caliber', domain=[(
        'record_type', '=', 'Caliber')], help="The Caliber of the product.", store=False)
    packaging_id = fields.Many2one('di.multi.table', string='Packaging', domain=[(
        'record_type', '=', 'Packaging')], help="The Packaging of the product.", store=False)
    ...
    printer_id = fields.Many2one(
        "di.printing.printer", string='Etiquette Printer', domain=[('isimpetiq', '=', True)])
    model_id = fields.Many2one(
        "di.printing.etiqmodel", string='Etiquette Model')
    ...
    date_dluo = fields.Date(
        string="DLUO Date", store=True, compute='_compute_dluo')
