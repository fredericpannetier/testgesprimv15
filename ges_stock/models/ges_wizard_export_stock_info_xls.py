...
#         return l1, l2
    ...
    def get_lines(self, data):#, warehouse):
        ...
        purchase_query = """
               ...
               AND 1 = %s AND p_o_l.product_id in %s group by p_o_l.product_id"""
        params = 1, product_ids if product_ids else (0, 0)
        ...
        for obj in categ_products:
            ...
            virtual_available = obj.virtual_available
...
#                 'outgoing': outgoing_qty,
                'qty': obj.qty_available,
                'packages': obj.product_tmpl_id.get_packs_qty(obj.qty_available),
                'pieces': obj.product_tmpl_id.get_pieces_qty(obj.qty_available),
                'nweight': obj.product_tmpl_id.get_nweight_qty(obj.qty_available),
...
#             sheet.write(9, p_col_no1 + 3, 'Outgoing', format21)
        sheet.write(9, p_col_no1, 'Quantity', format21)
        sheet.write(9, p_col_no1+1, 'Packages', format21)
        sheet.write(9, p_col_no1+2, 'Pieces', format21)
        sheet.write(9, p_col_no1+3, 'Net weight', format21)
...
#             sheet.merge_range(9, p_col_no1 + 8, 9, p_col_no1 + 9, 'Total Purchased', format21)
        sheet.write(9, p_col_no1 + 4, 'Valuation', format21)
        p_col_no1 = p_col_no1 + 11
        prod_row = 10
        prod_col = 0
#         for i in get_warehouse[1]:
        get_line = self.get_lines(d)
        for each in get_line:
            sheet.write(prod_row, prod_col, each['code'], font_size_8)
...
#                     sheet.write(prod_row, prod_col + 3, each['outgoing'], font_size_8)
            if each['qty'] < 0:
                sheet.write(prod_row, prod_col, each['qty'], red_mark)
            else:
                sheet.write(prod_row, prod_col, each['qty'], font_size_8)
            if each['packages'] < 0:
                sheet.write(prod_row, prod_col+1, each['packages'], red_mark)
            else:
                sheet.write(prod_row, prod_col+1, each['packages'], font_size_8)
            if each['pieces'] < 0:
                sheet.write(prod_row, prod_col+2, each['pieces'], red_mark)
            else:
                sheet.write(prod_row, prod_col+2, each['pieces'], font_size_8)
            if each['nweight'] < 0:
                sheet.write(prod_row, prod_col+3, each['nweight'], red_mark)
            else:
                sheet.write(prod_row, prod_col+3, each['nweight'], font_size_8)
...
#                                       font_size_8)
            if each['total_value'] < 0:
                ...
                sheet.write(prod_row, prod_col + 4, each['total_value'], font_size_8_r)
            prod_row = prod_row + 1
#             prod_row = 10
